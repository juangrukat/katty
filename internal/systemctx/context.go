...
func (sc *SystemContext) Prompt() string {
	...
	b.WriteString(`{"server":"SERVER","tool":"TOOL","args":{...},"final":false}` + "\n")
	b.WriteString("</katty_tool_call>\n\n")
	b.WriteString("The final field is optional and defaults to false.\n")
	b.WriteString("Set final=true only when the tool's stdout/stderr is itself the user's answer and Katty should print it directly, stop the turn, and avoid sending the output back to the model.\n")
	b.WriteString("Use final=true for terminal display requests such as list, cat, print, tail, grep, pwd, date, or whoami.\n")
	b.WriteString("Do not set final=true when the output is needed for further reasoning, debugging, fixing, summarizing, or diagnosis.\n")
	...
	b.WriteString("For terminal-like requests such as list, cat, print, run, tail, grep, pwd, date, or whoami, prefer katty.proc.exec with final=true so Katty can display stdout/stderr directly without a second interpretation pass.\n")
